document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnBase64').addEventListener('click', async () => {
    const file = document.getElementById('document').files[0];
    const username = document.getElementById('username').value;

    if (!file) {
      statusText.innerText = "Error: You must choose a file to encode into Base64!";
      return;
    }

    statusText.innerText = "Reading file and encoding to Base64...";
    const reader = new FileReader();
    reader.onload = async () => {
      const rawBase64 = reader.result.split(',')[1];
      statusText.innerText = "Sending Base64 payload across the network...";
